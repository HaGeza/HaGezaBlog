...
constexpr std::array<Vertex, (NumSections + 1) * NumRings> create_lathe_vertices(
    ...
    for (size_t ring = 0; ring < NumRings; ++ring) {
        for (size_t pt_ind = 0; pt_ind <= NumSections; ++pt_ind) {
            const Vec2 &pt = profile.at(pt_ind);
            float theta = static_cast<float>(ring) / static_cast<float>(NumRings) * 2.0 * PI;
            Vec3 position = Vec3{pt.x * constexpr_cos(theta), pt.y, pt.x * constexpr_sin(theta)};
...
template <std::size_t NumSections, std::size_t NumRings>
constexpr std::array<unsigned short, NumSections * NumRings * 6> create_lathe_indices(
    std::array<Vec2, NumSections + 1> profile) {
    std::array<unsigned short, NumSections * NumRings * 6> indices;
    size_t ind = 0;
